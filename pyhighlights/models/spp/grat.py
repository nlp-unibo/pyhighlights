...
from pyhighlights.models.spp.base import SPP, SPPData, SPPEmbedder, SPPEncoder, SPPPredictor
...
class GRATGuider(th.nn.Module):
    ...
    def forward(
        ...
        highlight_mask = highlight_mask.masked_fill_(~bool_mask[:, :, None], th.finfo(th.float).min)

        if self.training:
            # [bs, F, 1]
            attention_noises = th.normal(0,
                                         self.noise_sigma,
                                         highlight_mask.size(),
        ...
        # [bs, F, d_2]
        projected = self.projector.forward(encodings=encodings, attention_weights=highlight_mask)
...
class GRUGRATGuider(GRATGuider):
    ...
    def __init__(
        ...
        embedder = GRUEmbedder(vocab_size=vocab_size,
                               ...
                               embedding_matrix=embedding_matrix,
                               freeze_embeddings=freeze_embeddings)
        encoder_output_dim = encoder_hidden_size * 2 if bidirectional else encoder_hidden_size

        converter = GRATGuiderConverter(embedding_dim=embedding_dim,
        ...
        attention = GRATGuiderAttention(encoder_output_dim=encoder_output_dim,
                                        hidden_sizes=attention_hidden_sizes)

        projector = GRATGuiderProjector(encoder_output_dim=encoder_output_dim,
                                        hidden_sizes=projector_hidden_sizes)

        pooler = AttentionPooling(in_dim=encoder_output_dim,
